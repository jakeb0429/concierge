...
(async () => {
  ...
  for (let i = 0; i < 24; i++) {
    ...
    if (list.data.messages?.length) {
      { const probe = await gmail.users.messages.get({ userId: "me", id: list.data.messages[0].id, format: "metadata", metadataHeaders: ["Subject"] });
        const subj = probe.data.payload.headers?.find((h) => h.name === "Subject")?.value ?? "";
        const ageS = (Date.now() - Number(probe.data.internalDate)) / 1000;
        if (!subj.includes("—") || ageS > 180) { await new Promise((r) => setTimeout(r, 5000)); continue; } }
